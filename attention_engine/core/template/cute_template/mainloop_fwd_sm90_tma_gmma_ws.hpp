...
struct CollectiveMainloopFwd {
    ...
    mma(Params const& mainloop_params,
            ...
            for (int i = 0; i < size(tSrS); ++i) {
                if constexpr (!Is_causal) {  // Just masking based on col
                    ...
                    if (int(get<1>(tScS(i))) >= std::min(seqlen_k - n_block * kBlockN,
                        ...
                        // TODO: hack to avoid -INF
                        tSrS(i) = -1e10;// -INFINITY;
                    }
                }
            }
        }
        ...
        for (; n_block > 0; --n_block) {
            ...
            cute::copy(softmax.template online_fwd</*Is_first=*/false>(tSrS), scores_scale);
            warpgroup_wait<0>();
            pipeline_v.consumer_release(smem_pipe_read_v);  // release V
            ++smem_pipe_read_k;
            ++smem_pipe_read_v;
        ...
        // softmax.rescale_o(tOrO, scores_scale);
        softmax.template finalize_epilogue</*Check_inf=*/Is_causal>(tOrO);
        return;
    }
