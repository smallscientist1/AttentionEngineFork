...
class SigmoidTunner:
    ...
    def tune(self, kernel, BATCH, H, N_CTX, D_HEAD, D_HEADV, tuned_configs, file_path="tuned_result.json"):
        ...
        for mod, tuned_config in cached_results:
            try:
                # mod = tl.cached(kernel, output_idx_list, BATCH, H, N_CTX, D_HEAD, D_HEADV, *tuned_config.values())
                latency, tflops, ref_latency, ref_tflops  = bench_sigmoidattn_fwd(mod, BATCH, H, N_CTX, D_HEAD, D_HEADV)
            except Exception as e:
                print(traceback.format_exc())
                latency = 1e6
                tflops = 0
