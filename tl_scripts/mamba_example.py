...
def state_passing_triton(states, dA_chunk_cumsum, initial_states):
...
def chunk_cumsum_fwd(batch, seqlen, nheads):
    ...
    @T.prim_func
    def main(
        dt: T.Buffer((batch, seqlen, nheads), dtype),
        A: T.Buffer((nheads,), dtype),
        Output: T.Buffer((batch, nheads, nchunks), dtype)
    ):
        with T.Kernel(batch, nchunks, T.ceildiv(nheads, block_H), threads=128) as (bx, by, bz):
            pass
...
if __name__ == "__main__":
    ...
    # chunk_cumsum_fwd
    # program = chunk_cumsum_fwd(BATCH, SEQLEN, NHEADS)
    # mod, params = tl.lower(program)
    # mod = tl.Profiler(mod, params, [2], tl.TensorSupplyType.Normal)
    # mod.assert_allclose(chunk_cumsum_triton, rtol=0.01, atol=0.01)
    # latency = mod.do_bench(chunk_cumsum_triton, n_warmup=10, n_repeat=10, profiler="torch")
    ...
    # mod.assert_allclose(state_passing_ref, rtol=0.1, atol=0.1)
    # latency = mod.do_bench(state_passing_triton, n_warmup=10, n_repeat=10, profiler="torch")
    # print("{:.4f} ms".format(latency))
    # print("{:.2f} TFlops".format(total_flops / latency * 1e-9))
    # latency = mod.do_bench(mod, n_warmup=10, n_repeat=10, profiler="tvm")
    # print("{:.4f} ms".format(latency))
    ...
    # total_flops = 2 * BATCH * SEQLEN * NGROUPS * DSTATE * chunk_size
    # latency = mod.do_bench(bmm_triton, n_warmup=10, n_repeat=10, profiler="torch")
    # print("{:.4f} ms".format(latency))
    # print("{:.2f} TFlops".format(total_flops / latency * 1e-9))
    # latency = mod.do_bench(mod, n_warmup=10, n_repeat=10, profiler="tvm")
    # print("{:.4f} ms".format(latency))
    ...
    # latency = mod.do_bench(selective_state_update_triton, n_warmup=10, n_repeat=10, profiler="torch")
    # print("{:.4f} ms".format(latency))
    # print("{:.2f} TFlops".format(total_flops / latency * 1e-9))
    # latency = mod.do_bench(mod, n_warmup=10, n_repeat=10, profiler="tvm")
    # print("{:.4f} ms".format(latency))
