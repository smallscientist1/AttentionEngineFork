...
from .cute_template import CuteAttnTemplate
...
def lower_online_func(online_func, lower_cute_output: LowerCuteOutput):
    ...
    tl_code_online, input_vars_online = generate_tl_from_dag(list(new_online_rowscales.values()) + [scores_new, o_scalevar], to_tl=False, to_cute=True)

    # online_fwd_epilogue
    acco = SymbolicArray("acc_o_rowcol", Var("acc_o_rowcol"), shape_idx=["block_M", "dimv"])
    for k,v in online_rowscales.items():
    ...
    tl_code_online_epilogue, input_vars_online_epilogue = generate_tl_from_dag(list(new_final_rowscales.values()) + [acco_new], to_tl=False, to_cute=True)
    ...
    lower_cute_output.online_rowscales_vardefine += "\n".join([f"TensorT {var};" for var in new_final_rowscales.keys() if var not in online_rowscales.keys()])
    # maube  buggy because of acc_o_rowcol cannot inplace
    lower_cute_output.finalize_epilogue_body_vardefine = "\n".join([f"Tensor {var} = make_fragment_like({list(new_final_rowscales.keys())[0]});" for var in input_vars_online_epilogue if (var not in new_final_rowscales.keys() and var!="acc_o_rowcol") and var not in online_rowscales.keys()])
    ...
    lower_cute_output.final_rowscales_store_params_def = ", ".join([f"FrgTensorLSE const& {k}" for k in new_final_rowscales.keys()]) + ("," if new_final_rowscales else "")
    lower_cute_output.global_ptr_args = ", ".join([f"void* softmax_{k}_d" for k in new_final_rowscales.keys()]) + ("," if new_final_rowscales else "")
    # params.softmax_lse_ptr = softmax_lse_d;
    lower_cute_output.global_ptr_params_init = "\n".join([f"params.softmax_{k}_ptr = softmax_{k}_d;" for k in new_final_rowscales.keys()])
    ...
    # softmax_lse,
    lower_cute_output.final_rowscale_return = ", ".join([f"softmax_{k}" for k in new_final_rowscales.keys()]) + ("," if new_final_rowscales else "")
    # args.ptr_LSE, args.layout_LSE
    lower_cute_output.final_rowscales_return_struct = ", ".join([f"args.ptr_{k}, args.layout_{k}" for k in new_final_rowscales.keys()]) + ("," if new_final_rowscales else "")
    # if (thread_idx < seqlen_traits_q.actual_seq_len - m_block * kBlockM) { gLSE(thread_idx) = -INFINITY; }
    lower_cute_output.final_rowscales_store_code_write_zero = "\n".join([f"if (thread_idx < seqlen_traits_q.actual_seq_len - m_block * kBlockM) {{ g{k}(thread_idx) = -INFINITY; }}" for k in new_final_rowscales.keys()])
    # void * __restrict__ softmax_lse_ptr;
    lower_cute_output.global_ptr_params_def = "\n".join([f"void * __restrict__ softmax_{k}_ptr;" for k in new_final_rowscales.keys()])
        # static_cast<float*>(params.softmax_lse_ptr),
        #     // seqlen_traits_q.get_lse_gmem_layout(
        #     //     params.seqlen_q, params.h, params.b
        #     // )  // layout_LSE
    lower_cute_output.final_rowscales_params = ", ".join([f"static_cast<float*>(params.softmax_{k}_ptr), seqlen_traits_q.get_lse_gmem_layout( params.seqlen_q, params.h, params.b)" for k in new_final_rowscales.keys()]) # + ("," if new_final_rowscales else "")
