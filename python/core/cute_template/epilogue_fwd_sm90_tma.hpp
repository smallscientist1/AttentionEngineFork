...
struct CollectiveEpilogueFwd {
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_O, args.layout_O, {{final_rowscales_return_struct}} tma_store_O};
    }
    ...
    store_final(Params const& epilogue_params,
          FrgTensorO const& tOrO,
          // FrgTensorLSE const& lse,
          {{final_rowscales_store_params_def}}
          SharedStorage& shared_storage,
          TiledMma tiled_mma,
          int thread_idx,
          cute::tuple<int32_t, int32_t, int32_t> const& block_coord,
        ...
        Tensor taccOcO_row = taccOcO(make_coord(_0{}, _, _0{}), _, _0{});
        // CUTE_STATIC_ASSERT_V(size(lse) == size(taccOcO_row));                     // MMA_M
        {{final_rowscales_store_code_assert}}
        // if (get<1>(taccOcO_row(_0{})) == 0) {
        //     #pragma unroll
        //     for (int mi = 0; mi < size(lse); ++mi) {
        //         const int row = get<0>(taccOcO_row(mi));                
        ...
        //     }
        // }
        {{final_rowscales_store_code_write}}
    ...
    store_zero(
        ...
        {{final_rowscales_store_code_write_zero}}
    }
