...
def kernel(batch, heads, seq_len, dim, dimv, 
    ...
    def score_mod(
        ...
        scores: T.Buffer([block_M, block_N], accum_dtype), 
        mask: T.Buffer([block_M, block_N], accum_dtype), 
    ...
    def online_func(
        # scores: T.Buffer([block_M, block_N], accum_dtype),
        r_wo_clamp: T.Buffer([block_M], accum_dtype), 
        scores: T.Buffer([block_M, block_N], accum_dtype), 
        scores_0: T.Buffer([block_M, block_N], accum_dtype), 
    ...
    def main(
        ...
        g_mask: T.Buffer([1, heads, seq_len, seq_len_kv], dtype), 


        Output: T.Buffer(shape_v, dtype), # type: ignore
        g_r: T.Buffer([batch, heads, seq_len], accum_dtype), 
        ...
        with T.Kernel(T.ceildiv(seq_len, block_M), heads, batch, threads=thread_num) as (bx, by, bz):
            ...
            mask = T.alloc_fragment([block_M, block_N], dtype)
            ...
            # save final_rowscale
            T.copy(r, g_r[bz, by, bx * block_M : (bx + 1) * block_M])
...
def flashattn_bwd(batch, heads, seq_len, dim, dimv, is_casual, 
    ...
    def score_mod(
        ...
        mask: T.Buffer([block_M, block_N], accum_dtype), 
        qkT_0: T.Buffer([block_M, block_N], accum_dtype), 

        ):
        for i0,i1 in T.Parallel(block_M,block_N):
            qkT_0[i0,i1] = qkT[i0,i1] * mask[i0,i1]
        for i0,i1 in T.Parallel(block_M,block_N):
            qkT_0[i0,i1] = qkT_0[i0,i1] / 8.0
    ...
    def score_mod_backward(
        ...
        for i0,i1 in T.Parallel(block_M,block_N):
            mask[i0,i1] = dsT[i0,i1] * mask[i0,i1]
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_M), batch, threads=thread_num) as (bx, by, bz):
            ...
            r_shared = T.alloc_shared([1, block_N], accum_dtype)
            ...
            # score_mod_declare
            dsT = T.alloc_fragment([block_M, block_N], accum_dtype)
            mask = T.alloc_fragment([block_M, block_N], accum_dtype)
            qkT = T.alloc_fragment([block_M, block_N], accum_dtype)
            qkT_0 = T.alloc_fragment([block_M, block_N], accum_dtype)
            ...
            for k in T.Pipelined(loop_st, loop_ed, num_stages=1):
                ...
                # score_mod
                score_mod(qkT, mask, qkT_0) # qkT,

                # final_rowscales_load
                T.copy(g_r[bz, bx, k * block_N : (k + 1) * block_N], r_shared)


                # online_func_fwd
                for i0,i1 in T.Parallel(block_M,block_N):
                    qkT[i0,i1] = qkT[i0,i1] / r_shared[0,i1]

                
                # TODO: is causal
                if is_casual:
                    for i, j in T.Parallel(block_M, block_N):
                        qkT_0[i, j] = T.if_then_else(
                            by * block_M + i <= k * block_N + j, qkT_0[i, j], 0
                        )
                
                T.copy(dO[bz, k * block_N : (k + 1) * block_N, bx, :], do)
                T.copy(qkT_0, qkT_cast)
                ...
                # custom_bwd
                for i0,i1 in T.Parallel(block_M,block_N):
                    dsT[i0,i1] = dsT[i0,i1] / r_shared[0,i1]

                
                # score_mod_backward
                score_mod_backward(dsT, mask, qkT, qkT_0) #  qkT, 
