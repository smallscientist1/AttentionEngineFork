  ...
  if (256 <= ((int)threadIdx.x)) {
    ...
    for (int i_2 = 0; i_2 < 32; ++i_2) {
      for (int vec_s = 0; vec_s < 2; ++vec_s) {
        float condval;
        if ((((((i_2 >> 1) * 8) + ((((int)threadIdx.x) & 3) * 2)) + vec_s) <= ((((((int)blockIdx.x) * 128) + ((((int)threadIdx.x) >> 5) * 16)) + ((i_2 & 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) {
    ...
    for (int i_8 = 0; i_8 < 32; ++i_8) {
      uint1 __5;
      float2 v__8 = *(float2*)(scores + (i_8 * 2));
      ((half2*)(&(__5.x)))->x = (half_t)(v__8.x);
      ((half2*)(&(__5.x)))->y = (half_t)(v__8.y);
      *(uint1*)(acc_s_cast + (i_8 * 2)) = __5;
    }
    for (int k_1 = 0; k_1 < ((int)blockIdx.x); ++k_1) {
      ...
      for (int i_9 = 0; i_9 < 32; ++i_9) {
        for (int vec_s_1 = 0; vec_s_1 < 2; ++vec_s_1) {
          float condval_1;
          if (((((((k_1 * 128) + ((i_9 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + vec_s_1) + 128) <= ((((((int)blockIdx.x) * 128) + ((((int)threadIdx.x) >> 5) * 16)) + ((i_9 & 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) {
            condval_1 = 0.000000e+00f;
          } else {
            condval_1 = -CUDART_INF_F;
          }
          scores[((i_9 * 2) + vec_s_1)] = condval_1;
        }
      }
      tl::fence_proxy_async();
      ...
      for (int i_10 = 0; i_10 < 32; ++i_10) {
        float2 __6;
          float2 v__9 = *(float2*)(acc_o + (i_10 * 2));
          float2 v__10 = make_float2(o_scale[(i_10 & 1)], o_scale[(i_10 & 1)]);
          __6.x = (v__9.x*v__10.x);
          __6.y = (v__9.y*v__10.y);
        *(float2*)(acc_o + (i_10 * 2)) = __6;
      }
      tl::fence_proxy_async();
      ...
      for (int i_16 = 0; i_16 < 32; ++i_16) {
        uint1 __11;
        float2 v__19 = *(float2*)(scores + (i_16 * 2));
        ((half2*)(&(__11.x)))->x = (half_t)(v__19.x);
        ((half2*)(&(__11.x)))->y = (half_t)(v__19.y);
        *(uint1*)(acc_s_cast + (i_16 * 2)) = __11;
      }
    }
    #pragma unroll
    for (int i_17 = 0; i_17 < 32; ++i_17) {
      float2 __12;
        float2 v__20 = *(float2*)(acc_o + (i_17 * 2));
        float2 v__21 = make_float2(o_scale[(i_17 & 1)], o_scale[(i_17 & 1)]);
        __12.x = (v__20.x*v__21.x);
        __12.y = (v__20.y*v__21.y);
      *(float2*)(acc_o + (i_17 * 2)) = __12;
    }
    tl::fence_proxy_async();
