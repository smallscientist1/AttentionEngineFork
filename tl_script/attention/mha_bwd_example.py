# bug: nan for causal --fixed
# bug: TMA load lse_shared

# fa2 vs tl
# torch: 0.28 ms
# torch: 154.80 TFlops
# tl: 0.57 ms
# tl: 75.74 TFlops --128,128-> 83 tflops --thread256--> 86.34 TFlops
import argparse
import torch
from tvm import tl
...
def flashattn_bwd(batch, heads, seq_len, dim, is_casual, block_M, block_N):
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_M), batch, threads=256) as (bx, by, bz):
            ...
            lse_local = T.alloc_fragment([block_N], accum_dtype)
            delta = T.alloc_shared([block_N], accum_dtype)
            delta_local = T.alloc_fragment([block_N], accum_dtype)
            ...
            for k in T.Pipelined(loop_st, loop_ed, num_stages=2):
                ...
                T.copy(lse[bz, bx, k * block_N : (k + 1) * block_N], lse_local)
                T.copy(lse_local, lse_shared)
                # if is_casual:
                #     for i, j in T.Parallel(block_M, block_N):
                #         qkT[i, j] = T.if_then_else(
                #             by * block_M + i <= k * block_N + j, qkT[i, j], -T.infinity(accum_dtype)
                #         )
                for i, j in T.Parallel(block_M, block_N):
                    qkT[i, j] = T.exp2(qkT[i, j] - lse_shared[j])# lse_shared[j])
                if is_casual:
                    for i, j in T.Parallel(block_M, block_N):
                        qkT[i, j] = T.if_then_else(
                ...
                T.copy(delta_local, delta)
                T.clear(dsT)
                T.gemm(V_local, do, dsT, transpose_B=True, policy=T.GemmWarpPolicy.FullRow)

                for i, j in T.Parallel(block_M, block_N):
                    dsT_cast[i, j] = qkT[i, j] * (dsT[i, j] - delta[j]) * sm_scale # delta_local
...
if __name__ == "__main__":
    ...
    if casual:
        total_flops *= 0.5
    Q = (
        torch.randn(BATCH, N_CTX, H, D_HEAD, dtype=torch.half, device="cuda")
        # .normal_()
        .requires_grad_()
    )
    K = torch.randn_like(Q).requires_grad_() # empty_like(Q).normal_().requires_grad_()
