...
# TL_MAIN = """
    ...
    def score_mod(
        ...
        for i0,i1 in T.Parallel(block_M,block_N):
            scores[i0,i1] = scores[i0,i1] * float(0.5)
        for i0,i1 in T.Parallel(block_M,block_N):
            fast_tanh(scores[i0,i1], scores[i0,i1])
        for i0,i1 in T.Parallel(block_M,block_N):
            scores[i0,i1] = scores[i0,i1] + float(1)
        for i0,i1 in T.Parallel(block_M,block_N):
            scores[i0,i1] = scores[i0,i1] * float(0.5)
...
# TL_MAIN_BWD = """
    ...
    def score_mod(
        ...
        softmax_bias: T.Buffer([1], accum_dtype), 
        qkT_0_0_0_0: T.Buffer([block_M, block_N], accum_dtype), 

        ):
        for i0,i1 in T.Parallel(block_M,block_N):
            qkT[i0,i1] = qkT[i0,i1] + softmax_bias[0]
        for i0,i1 in T.Parallel(block_M,block_N):
            qkT[i0,i1] = qkT[i0,i1] * float(0.5)
        for i0,i1 in T.Parallel(block_M,block_N):
            fast_tanh(qkT[i0,i1], qkT[i0,i1])
        for i0,i1 in T.Parallel(block_M,block_N):
            qkT_0_0_0_0[i0,i1] = qkT[i0,i1] + float(1)
        for i0,i1 in T.Parallel(block_M,block_N):
            qkT_0_0_0_0[i0,i1] = qkT_0_0_0_0[i0,i1] * float(0.5)
    ...
    def score_mod_backward(
        ...
        dsT_0_1_0: T.Buffer([1], accum_dtype), 
        qkT: T.Buffer([block_M, block_N], accum_dtype), 
        softmax_bias: T.Buffer([1], accum_dtype), 
        qkT_0_0_0_0: T.Buffer([block_M, block_N], accum_dtype), 
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_M), batch, threads=thread_num) as (bx, by, bz):
            ...
            dsT_0_1_0 = T.alloc_fragment([1], accum_dtype)
            qkT = T.alloc_fragment([block_M, block_N], accum_dtype)
            softmax_bias = T.alloc_fragment([1], accum_dtype)
            qkT_0_0_0_0 = T.alloc_fragment([block_M, block_N], accum_dtype)
            ...
            for k in T.Pipelined(loop_st, loop_ed, num_stages=2):
                ...
                # score_mod
                score_mod(qkT, softmax_bias, qkT_0_0_0_0) # qkT,
                ...
                if is_casual:
                    for i, j in T.Parallel(block_M, block_N):
                        qkT_0_0_0_0[i, j] = T.if_then_else(
                            by * block_M + i <= k * block_N + j, qkT_0_0_0_0[i, j], 0
                        )
                ...
                # score_mod_backward
                score_mod_backward(dsT, dsT_0_0, dsT_0_1_0, qkT, softmax_bias, qkT_0_0_0_0) #  qkT, 
...
def eval():
    ...
    for B, H, S, D in BHSD:
        print("B, H, S, D", B, H, S, D)
        do_bench_sigmoidattn(attention, B, H, S, D, D, requires_grad=True)
