...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal):
        ...
        mod = tl.utils.cached(flashattn_fwd, [3, 4], BATCH, H, N_CTX, D_HEAD, causal, block_M, block_N)
        o, lse = mod(q, k, v)
    ...
    def backward(ctx, do):
        ...
        block_M = 128
        block_N = 64 # 128 if D_HEAD <= 64 else 32
        mod_prep = tl.utils.cached(flashattn_bwd_preprocess, [2], BATCH, H, N_CTX, D_HEAD)
        mod_post = tl.utils.cached(flashattn_bwd_postprocess, [1], BATCH, H, N_CTX, D_HEAD)
        delta = mod_prep(o, do)
        mod = tl.utils.cached(
