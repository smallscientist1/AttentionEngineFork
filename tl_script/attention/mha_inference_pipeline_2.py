...
def flashattn(batch, heads, seqlen_q, seqlen_kv, dim, is_casual, block_M, block_N):
    ...
    def combine(
        ...
        with T.Kernel(T.ceildiv(seqlen_q, block_M), heads, batch, threads=128) as (bx, by, bz):
            ...
            lse_logsum_local = T.alloc_fragment([num_split, block_M], accum_dtype)
            logsum_accum_local = T.alloc_fragment([block_M], accum_dtype)
            ...
            for k,i in T.Parallel(num_split, block_M):
                lse_logsum_local[k, i] = T.exp2(lse_local[k, i] - lse_max_local[i])
            T.reduce_sum(lse_logsum_local, logsum_accum_local, dim=0)
            ...
            # for k in T.serial(num_split): # for ablation
                ...
                for i in T.Parallel(block_M):
                    scale_local[i] = T.exp2(lse_local_split[i] - logsum_accum_local[i])
                for i, j in T.Parallel(block_M, dim):
