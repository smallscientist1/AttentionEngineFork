...
def chunk_fwd_h(
    ...
    def main(
        ...
        dt: T.Buffer((batch,heads,seq_len), dtype),
        ...
        with T.Kernel(NK, NV, batch * head, threads=num_threads) as (bx, by, bz):
            ...
            dt_local = T.alloc_fragment((BT,), dtype=accum_dtype)
            ...
            for i_t in T.Pipelined(NT, num_stages=num_stages):
                ...
                for i0,i1 in T.Parallel(BT,BK):
                    b_k[i0,i1] = b_k[i0,i1] * dt_local[i0]
...
def chunk_o(
    ...
    def main(
        ...
        with T.Kernel(NV, NT, batch * head, threads=num_threads) as (bx, by, bz):
            ...
            dt_local = T.alloc_fragment((1,BT,), dtype=accum_dtype)
            ...
            T.copy(dt_shared, dt_local)
            for i0,i1 in T.Parallel(BT,BT):
                bs[i0,i1] = bs[i0,i1] * dt_local[0,i1]
...
class LinearAttention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, decay, A, dt, ):
