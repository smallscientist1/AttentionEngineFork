        ...
        with T.Kernel(NV, NT, batch * head, threads=num_threads) as (bx, by, bz):
            ...
            for ik in T.Pipelined(NK, num_stages=num_stages):
                ...
                for i0,i1 in T.Parallel(BT,BK):
                    bq[i0,i1] = bq[i0,i1] * float(0.08838834764831843)
...
class LinearAttention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, decay,  ):
        ...
        o = chunk_fwd_o_mod(h, q, k, v, decay_cumsum, )

        ctx.save_for_backward(q, k, v, decay_cumsum,  )
        ctx.BT = BT
        return o
    ...
    def backward(ctx, do):
        BT = ctx.BT
        q, k, v, decay_cumsum,  = ctx.saved_tensors
        ...
        q = q * float(0.08838834764831843)
        h = chunk_fwd_h_mod(k, v, decay_cumsum, )
        dh = chunk_bwd_dh_mod(q, k, v, decay_cumsum, do)
        dq, dk, dg = chunk_bwd_dqkg_mod(q, k, v, h, decay_cumsum, do, dh)
        if HQ < H:
            dq = einops.einsum(dq, "b (g h) t d -> b g t d", g=HQ)
        if HK < H:
            dk = einops.einsum(dk, "b (g h) t d -> b g t d", g=HK)
        dg = dg.sum(0)
        ...
        dq = dq*float(0.08838834764831843)
...
if __name__ == "__main__":
    B, H, S, D, DV = 16, 8, 2048, 128, 128 # bug 16384
    dtype = torch.bfloat16
    
    from benchmark.bench_utils import do_bench_simple_gla
    do_bench_simple_gla(linear_attention, B, H, S, D, DV, BT=64,requires_grad=True)
