...
def _chunk_fwd_h(
        batch, head, seqlen, dim, dimv, BT
):
    @autotune(configs=get_configs(), keys=['BK', 'BV', 'num_stages', 'thread_num'], warmup=10, rep=20)
    @jit(out_idx=[3,], supply_type=tl.TensorSupplyType.Normal, ref_prog=None, rtol=0.01, atol=0.01)
    def kernel(
...
if __name__ == "__main__":
    ...
    final_state = False
    # best_latency, best_config, ref_latency = _chunk_fwd_h(B, H, TLen, D, DV, BT)
    # print(best_latency, best_config, ref_latency)

    f = open("chunk_fwd_h.csv", "w")
    writer = csv.writer(f)
